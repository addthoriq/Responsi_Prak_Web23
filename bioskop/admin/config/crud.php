<?php 
	...
	class input{
		function input($type=null,$name=null,$val=null,$property=null){
			$input = "<input type='$type' name='$name' value='$val' $property>";
			return $input;
		}
		function textarea($name=null,$val=null,$property){
			$input	= "<textarea name='$name' $property>$val</textarea>";
			return $input;
		}
		function select($name=null,$jml=null,$val=null,$property=null,$check=null){
			$op = "";
			for($x=1; $x <=$jml; $x++){
				$xxx="";
				if ($val[$x] == $check){
			...
			$input="<select name='$name' $property>$op></select>";
			return $input;
		}
		function newselect($name=null,$jml=null,$val=null,$serc=null,$property=null,$check=null){
			$op = "";
			for($x=1; $x <=$jml; $x++){
				$xxx="";
				if ($serc[$x] == $check){
